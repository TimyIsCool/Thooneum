...
# thooneum_hash
RUN git clone https://github.com/thooneum/thooneum_hash
RUN cd thooneum_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} thooneum
RUN useradd -u ${USER_ID} -g thooneum -s /bin/bash -m -d /thooneum thooneum
...
RUN mkdir /thooneum-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /thooneum-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /thooneum-src
