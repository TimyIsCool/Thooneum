...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // users ignore all instructions on thooneumcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in thooneum.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("thooneum.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in thooneum.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Thooneum features */
    { "thooneum",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "thooneum",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "thooneum",               "gobject",                &gobject,                true,  {} },
    { "thooneum",               "voteraw",                &voteraw,                true,  {} },
