...
const char * const BITCOIN_CONF_FILENAME = "thooneum.conf";
const char * const BITCOIN_PID_FILENAME = "thooneumd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ThooneumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ThooneumCore
    // Mac: ~/Library/Application Support/ThooneumCore
    // Unix: ~/.thooneumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ThooneumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ThooneumCore";
#else
    // Unix
    return pathRet / ".thooneumcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty thooneum.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override thooneum.conf
