...
// which are part of the Thooneum repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="thooneum-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /thooneum-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "thooneum-thooneum/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from thooneum-thooneum/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'thooneum-thooneum/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from thooneum-thooneum/develop"
            }
          }

          if (hasCache) {
            sh "cd /thooneum-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /thooneum-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /thooneum-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /thooneum-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /thooneum-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /thooneum-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /thooneum-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
