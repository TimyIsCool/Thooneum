...
PACKAGE_VERSION=$($SRC_PATH/src/thooneumd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/thooneumd $DEB_BIN
cp $SRC_PATH/src/thooneum-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/thooneum-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/thooneumd.1 $DEB_MAN
cp $SRC_DOC/man/thooneum-cli.1 $DEB_MAN
cp $SRC_DOC/man/thooneum-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/thooneumd.bash-completion $DEB_CMP/thooneumd
cp $SRC_PATH/contrib/thooneum-cli.bash-completion $DEB_CMP/thooneum-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/thooneumd.1
gzip --best -n $DEB_MAN/thooneum-cli.1
gzip --best -n $DEB_MAN/thooneum-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/thooneumd $DEB_BIN/thooneum-cli
